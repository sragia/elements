...
local editBox = EL:GetModule('edit-box-input')
...
local function ConfigureFrame(f, options)
    ...
    input:SetFont(EL.const.fonts.DEFAULT, 11, 'OUTLINE')
    ...
    bgTex:SetTexture(EL.const.textures.frame.inputs.editboxBg)
    ...
    label:SetFont(EL.const.fonts.DEFAULT, 10, 'OUTLINE')
    ...
    local onHover = EL.utils.animation.fade(hoverContainer, 0.15, 0.1, 1)
    local onLeave = EL.utils.animation.fade(hoverContainer, 0.15, 1, 0.1)
    ...
    f.SetElementData = function(self, option, element)
        self.optionData = option
        self.element = element
        self:SetLabel(option.label)
        self:SetEditorValue(element[option.name] or '')
    end
