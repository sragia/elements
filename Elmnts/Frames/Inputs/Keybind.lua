...
---@class ELKeybind
local keybind = EL:GetModule('keybind-frame')
...
local SetupKeyListening = function(f)
    ...
    local function onKeyDown(self, key)
        ...
        if (RESERVED_KEYS[key]) then
            EL.utils.printOut('Reserved key ' .. key .. ' pressed')
            return
        end
...
local SetupFrame = function(f)
    ...
    bg:SetTexture(EL.const.textures.frame.inputs.buttonBg)
    ...
    keybind:SetFont(EL.const.fonts.DEFAULT, 13, 'OUTLINE')
    ...
    hoverTexture:SetTexture(EL.const.textures.frame.inputs.buttonHover)
    ...
    local onHover = EL.utils.animation.fade(hover, 0.1, 0, 1)
    local onLeave = EL.utils.animation.fade(hover, 0.1, 1, 0)
    f.onHover = onHover
    f.onLeave = onLeave
    ...
    name:SetFont(EL.const.fonts.DEFAULT, 12, 'OUTLINE')
    name:SetPoint('LEFT')
    name:SetWidth(0)
    f.name = name
