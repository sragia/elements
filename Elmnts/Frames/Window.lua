...
local windowManager = EL:GetModule('window-manager')
...
local configure = function(frame)
    ...
    frame.fadeIn = EL.utils.animation.fade(frame, 0.2, 0, 1)
    frame.fadeOut = EL.utils.animation.fade(frame, 0.2, 1, 0)
    frame.fadeOut:SetScript('OnFinished', function() frame:Hide() end)
    EL.utils.animation.diveIn(frame, 0.2, 0, 20, 'IN', frame.fadeIn)
    EL.utils.animation.diveIn(frame, 0.2, 0, -20, 'OUT', frame.fadeOut)
    ...
    if (not frame.logo) then
        ...
        texture:SetTexture(EL.const.textures.logo)
        ...
        version:SetFont(EL.const.fonts.DEFAULT, 10, 'OUTLINE')
    ...
    if (not frame.resizeBtn) then
        ...
        resizeBtn:SetNormalTexture(EL.const.textures.frame.resizeBtn)
        resizeBtn:SetHighlightTexture(EL.const.textures.frame.resizeBtnHighlight)
    ...
    if (not frame.close) then
        ...
        texture:SetTexture(EL.const.textures.frame.inputs.buttonBg)
        ...
        closeIcon:SetTexture(EL.const.textures.frame.closeIcon)
    ...
    if (not frame.timer) then
        ...
        timerBg:SetTexture(EL.const.textures.frame.statusBar)
        ...
        frame.timer:SetStatusBarTexture(EL.const.textures.frame.statusBar)
    ...
    frame.title = title
    title:SetFont(EL.const.fonts.DEFAULT, 10, 'OUTLINE')
    title:SetTextColor(1, 1, 1)
    title:SetPoint('CENTER', frame, 'TOP')
    title:SetText('elmnts')
